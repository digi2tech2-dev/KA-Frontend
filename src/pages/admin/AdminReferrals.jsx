...
import agentProofImage from '../../assets/slide-3.jpg';
...
const REFERRAL_RATE_STORAGE_KEY = 'kacard_admin_referral_commission_rate';
...
const AdminReferrals = () => {
  ...
  const statCards = [
    { label: isArabic ? 'أصحاب أكواد الإحالة' : 'Referral owners', value: stats.owners, icon: Share2, iconClass: 'bg-cyan-500/10 text-cyan-500' },
    { label: isArabic ? 'إجمالي المدعوين' : 'Total invited', value: stats.invited, icon: UserRoundPlus, iconClass: 'bg-cyan-500/10 text-cyan-500' },
    ...
    <div className="admin-referrals-page mx-auto w-full max-w-[1500px] space-y-3 pb-10" dir={dir}>
            ...
            return (
              <article key={request.id} className="border border-[#14532d]/25 border-s-4 border-s-[#fb923c] bg-white/60 p-3 shadow-[4px_4px_0_#14532d14] dark:bg-[#091b20]/65">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0 flex-1"><div className="flex items-center gap-1.5"><p className="truncate text-xs font-black text-[var(--color-text)]">{request.name}</p>{request.isTest ? <span className="rounded-full bg-cyan-500/10 px-1.5 py-0.5 text-[0.5rem] font-black text-cyan-500">{isArabic ? 'تجريبي' : 'Demo'}</span> : null}</div><p dir="ltr" className="truncate text-left text-[0.58rem] text-[var(--color-text-secondary)]">{request.email}</p></div>
                ...
                <a href={request.proofImage} target="_blank" rel="noreferrer" className="mt-2 flex items-center gap-2 rounded-lg border border-cyan-400/18 bg-cyan-500/6 p-2 transition hover:border-cyan-400/35"><img src={request.proofImage} alt={isArabic ? 'صورة إثبات العملاء' : 'Customer proof'} className="h-12 w-20 rounded-md object-cover" /><div><p className="text-[0.58rem] font-black text-[var(--color-text)]">{isArabic ? 'صورة إثبات وجود عملاء' : 'Customer proof image'}</p><p className="text-[0.5rem] font-semibold text-cyan-500">{isArabic ? 'اضغط لعرض الصورة كاملة' : 'Open full image'}</p></div></a>
      ...
      <section className={`${activePanel === 'earnings' ? '' : 'hidden'} overflow-hidden rounded-xl border border-[color:rgb(var(--color-border-rgb)/0.55)] bg-[color:rgb(var(--color-card-rgb)/0.82)] shadow-[0_18px_50px_-42px_rgb(var(--color-primary-rgb)/0.7)]`}>
        <div className="flex items-center justify-between gap-2 border-b border-[color:rgb(var(--color-border-rgb)/0.5)] p-2">
          ...
          <div className="flex min-w-0 flex-1 justify-end gap-0.5">
            <label className="relative min-w-0 max-w-24 flex-1"><Search className="absolute start-0.5 top-1/2 h-1.5 w-1.5 -translate-y-1/2 text-[var(--color-muted)]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={isArabic ? 'ابحث بالاسم أو الكود...' : 'Search name or code...'} className="h-3 w-full rounded-full border border-[color:rgb(var(--color-border-rgb)/0.65)] bg-[var(--color-surface)] pe-0.5 ps-2.5 text-[0.31rem] font-bold leading-none text-[var(--color-text)] outline-none focus:border-cyan-400" /></label>
        ...
        <div className="divide-y divide-[color:rgb(var(--color-border-rgb)/0.42)]">
          {visibleOwners.map((owner) => {
            ...
            return <article
              ...
              className="grid cursor-pointer grid-cols-3 gap-x-2 gap-y-3 p-3 transition-all hover:bg-[color:rgb(var(--color-primary-rgb)/0.055)] focus-visible:bg-[color:rgb(var(--color-primary-rgb)/0.07)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/45 lg:grid-cols-[minmax(240px,1.3fr)_repeat(3,minmax(120px,.65fr))_auto] lg:items-center lg:gap-4 sm:p-5"
            >
              <div className="col-span-2 row-start-1 flex min-w-0 items-center gap-2 lg:col-span-1 lg:row-auto lg:gap-3"><img src={owner.avatar} alt={owner.name} className="h-10 w-10 rounded-full border-2 border-cyan-400/25 object-cover shadow-md lg:h-12 lg:w-12" /><div className="min-w-0"><div className="flex items-center gap-1.5"><p className="truncate text-xs font-black text-[var(--color-text)] lg:text-sm">{owner.name}</p>{owner.isTest ? <span className="rounded-full bg-cyan-500/10 px-1.5 py-0.5 text-[0.48rem] font-black text-cyan-500 lg:px-2 lg:text-[0.55rem]">{isArabic ? 'تجريبي' : 'Demo'}</span> : null}</div><p dir="ltr" className="truncate text-left text-[0.56rem] font-semibold text-[var(--color-text-secondary)] lg:text-[0.66rem]">{owner.email}</p><span dir="ltr" className="mt-0.5 inline-flex rounded-md bg-cyan-500/10 px-1.5 py-0.5 font-mono text-[0.52rem] font-black tracking-wider text-cyan-500 lg:mt-1 lg:px-2 lg:text-[0.62rem]">{owner.code}</span></div></div>
              ...
              <div className="col-start-3 row-start-2 min-w-0 lg:col-auto lg:row-auto"><p className="truncate text-[0.5rem] font-bold text-[var(--color-text-secondary)] lg:text-[0.6rem]">{isArabic ? 'متاح للسحب' : 'Available'}</p><p dir="ltr" className="mt-0.5 truncate text-end text-xs font-black text-cyan-500 lg:mt-1 lg:text-base">{available.toLocaleString('en-US')} <span className="text-[0.52rem] lg:text-[0.62rem]">{owner.currency}</span></p><p className="mt-0.5 truncate text-[0.46rem] font-bold text-[var(--color-text-secondary)] lg:mt-1 lg:text-[0.56rem]">{isArabic ? `تم سحب ${owner.withdrawn.toLocaleString('en-US')}` : `${owner.withdrawn.toLocaleString('en-US')} withdrawn`}</p></div>
              <button type="button" onClick={() => setSelectedOwner(owner)} aria-label={isArabic ? 'عرض التفاصيل' : 'View details'} className="col-start-3 row-start-1 inline-flex h-9 items-center justify-self-end gap-1 rounded-lg border border-cyan-400/20 bg-cyan-500/10 px-2 text-[0.55rem] font-black text-cyan-500 transition hover:bg-cyan-500/15 lg:col-auto lg:row-auto lg:h-10 lg:justify-self-auto lg:gap-2 lg:rounded-xl lg:px-3 lg:text-xs"><Eye className="h-3.5 w-3.5 lg:h-4 lg:w-4" /><span className="hidden sm:inline">{isArabic ? 'التفاصيل' : 'Details'}</span><ChevronLeft className="hidden h-3.5 w-3.5 rtl:rotate-0 ltr:rotate-180 lg:block" /></button>
            </article>;
          })}
      ...
      <Modal isOpen={Boolean(selectedOwner)} onClose={() => setSelectedOwner(null)} title={isArabic ? 'تفاصيل كود الإحالة' : 'Referral details'} size="xl">
        {selectedOwner ? <div className="space-y-4" dir={dir}>
          <div className="flex items-center gap-3 rounded-2xl bg-[color:rgb(var(--color-primary-rgb)/0.07)] p-4"><img src={selectedOwner.avatar} alt={selectedOwner.name} className="h-14 w-14 rounded-full object-cover" /><div className="min-w-0 flex-1"><p className="font-black text-[var(--color-text)]">{selectedOwner.name}</p><p dir="ltr" className="truncate text-left text-xs text-[var(--color-text-secondary)]">{selectedOwner.email}</p></div><span dir="ltr" className="rounded-lg bg-cyan-500/12 px-3 py-1.5 font-mono text-xs font-black text-cyan-500">{selectedOwner.code}</span></div>

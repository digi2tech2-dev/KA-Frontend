...
import { ClipboardCopy, ExternalLink, KeyRound, RefreshCw, ShieldCheck } from 'lucide-react';
...
const DeveloperApi = () => {
  ...
  const copyToken = async () => {
    try {
      ...
      addToast(copied ? 'Token copied.' : 'Unable to copy token.', copied ? 'success' : 'error');
    } catch {
      addToast('Unable to copy token.', 'error');
    }
  };
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5 px-4 pb-16 sm:px-6 lg:px-8">
      <Card className="rounded-2xl border border-[color:rgb(var(--color-primary-rgb)/0.3)] bg-[color:rgb(var(--color-card-rgb)/0.92)] p-6 shadow-[var(--shadow-subtle)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--color-text-secondary)]">Generate an API token and control allowed source IP addresses. The integration reference is available on a separate public page.</p>
          </div>
          <Link to="/api-docs" className="inline-flex items-center justify-center gap-2 rounded-lg border border-[color:rgb(var(--color-primary-rgb)/0.35)] px-4 py-2 text-sm font-semibold text-[var(--color-primary)] transition hover:bg-[color:rgb(var(--color-primary-rgb)/0.08)]">Open API Documentation <ExternalLink className="h-4 w-4" /></Link>
        </div>
      </Card>
      ...
      <Card className="rounded-2xl p-6 shadow-[var(--shadow-subtle)]">
        ...
        <p className="mt-2 text-sm text-[var(--color-text-secondary)]">Leave the IP list empty to allow requests from any address. Enter one address per line or separate values with commas.</p>
        <label className="mt-4 block text-sm font-semibold text-[var(--color-text)]" htmlFor="api-whitelist">Allowed IP addresses</label>
        <textarea id="api-whitelist" value={whitelistText} onChange={(event) => setWhitelistText(event.target.value)} rows={5} disabled={!apiEnabled} className="mt-2 w-full rounded-xl border border-[color:rgb(var(--color-border-rgb)/0.8)] bg-[color:rgb(var(--color-elevated-rgb)/0.55)] p-3 text-sm text-[var(--color-text)]" />
        <label className="mt-4 block text-sm font-semibold text-[var(--color-text)]" htmlFor="api-webhook">Webhook URL</label>
        <input id="api-webhook" value={webhookUrl} onChange={(event) => setWebhookUrl(event.target.value)} disabled={!apiEnabled} placeholder="https://example.com/webhook" className="mt-2 w-full rounded-xl border border-[color:rgb(var(--color-border-rgb)/0.8)] bg-[color:rgb(var(--color-elevated-rgb)/0.55)] px-3 py-2 text-sm text-[var(--color-text)]" />
        <div className="mt-4"><Button type="button" disabled={isSaving || !apiEnabled} onClick={saveSettings}>Save API settings</Button></div>
      </Card>
    </div>
  );
};

...
| --- | --- | --- | --- |
| `/` | `PublicCatalog` | Public | Public storefront catalog. |
...
`PublicCatalog` works without login. In real mode it calls the public catalog/product/category fallback paths. In mock mode it uses seeded product and category data. Product images are resolved through `resolveImageUrl()`, which handles absolute URLs, data/blob URLs, and relative upload paths.
